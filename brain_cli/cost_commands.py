...
app = typer.Typer(
    help="Track and analyze Azure OpenAI costs",
...
@app.command()
def summary(
    days: Optional[int] = typer.Option(30, "--days", "-d", help="Number of days to analyze"),
    month: Optional[str] = typer.Option(None, "--month", "-m", help="Specific month (YYYY-MM format)")
) -> None:
    ...
    try:
        ...
        console.print(Panel(
            summary_text,
            title=f"Brain Tool Cost Summary ({period_desc})",
            border_style="blue"
        ))
...
def breakdown(
    ...
    for operation, data in sorted_ops:
        ...
        console.print(Panel(
            op_text,
            title=operation.title().replace('_', ' '),
            border_style="cyan"
        ))
...
def export(
    ...
    days: Optional[int] = typer.Option(None, "--days", "-d", help="Number of days to export"),
    month: Optional[str] = typer.Option(None, "--month", "-m", help="Specific month (YYYY-MM)")
) -> None:
...
@app.command()
def pricing(
    model: Optional[str] = typer.Option(None, "--model", "-m", help="Show pricing for specific model"),
    ...
    if update:
        console.print("[yellow]Pricing updates not yet implemented. Please update manually in cost_tracker.py[/yellow]")
        return
    ...
    for model_name, prices in pricing_data.items():
        pricing_table.add_row(
            model_name,
            f"${prices['input'] * 1000:.6f}",
            f"${prices['output'] * 1000:.6f}"
        )
