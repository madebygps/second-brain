...
from .llm_analysis import _truncate_text, generate_semantic_backlinks
from .llm_client import LLMClient

logger = logging.getLogger(__name__)


def _extract_report_themes(
    entries: list[DiaryEntry], llm_client: LLMClient, top_n: int = DEFAULT_THEMES_COUNT
) -> list[str]:
...
{combined_text}"""
    ...
    try:
        ...
        logger.debug(
            f"Extracted {len(themes)} themes in {elapsed:.2f}s from {len(entries)} entries"
        )
    ...
    for entry in sorted_entries:
        ...
        if candidates:
            ...
            semantic_links = generate_semantic_backlinks(
                entry, candidates, llm_client, max_links=MAX_SEMANTIC_LINKS
            )
            if semantic_links:
                entry_connections[entry.date.isoformat()] = len(semantic_links)

    # Sort by number of connections
    top_connected = sorted(entry_connections.items(), key=lambda x: x[1], reverse=True)[
        :TOP_CONNECTED_ENTRIES
    ]
